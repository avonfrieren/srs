...
- `SheetImporter.cs` — téléchargement des trois onglets + caches `Saves/srs/asides.csv`, `bsides.csv` et `farewell.csv` (écriture atomique), rechargés au `Load()` ⇒ hors-ligne OK. Ces caches **sont** aussi le fallback manuel : des CSV déposés à la main y sont relus tels quels. Réseau uniquement sur le bouton, jamais au lancement, jamais sur le thread de jeu. Mise à jour tout-ou-rien : un onglet qui échoue n'écrit aucun cache.
