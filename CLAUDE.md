...
- `TierComparison.cs` — ligne temps + palier sous le timer, dessinée après `orig` de `SpeedrunTimerDisplay.Render` ; **décalée d'une ligne si srta est chargé**. Largeur du fond = la formule de SpeedrunTool (`60 + 18*(len-8)`), surtout pas la largeur mesurée du texte (le dégradé `strawberryCountBG` doit passer *sous* la fin du texte).
