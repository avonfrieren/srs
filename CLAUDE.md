...
`cd Tests && ~/.dotnet/dotnet test` (xUnit). Le projet de test `<Compile Include>` directement les fichiers source game-free (`SheetData.cs`, `SegmentAutoDetect.Names.cs`, `SheetUrls.cs`) au lieu de référencer `srs.dll` ⇒ aucune dépendance à Celeste, et les membres `internal` sont visibles sans `InternalsVisibleTo`. **Corollaire : garder ces deux fichiers sans `using` du jeu.**
...
3. Réseau → jamais sur le thread de jeu, timeout, erreurs loggées jamais levées (rafraîchissement automatique au lancement depuis la v3.5.0).
